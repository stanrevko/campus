...
//        'primaryKey' => 'id',
    ...
    public function setRelativeAttributes($action = '')
    {
        ...
        {// relation example; array(3) { [0]=> string(18) "CBelongsToRelation" [1]=> string(4) "Kind" [2]=> string(7) "kind_id" }
            if ($relation[0] == CActiveRecord::BELONGS_TO)
            {              
                $number = array_search($key, $this->notUseTitleOfRelation);
                ///если есть в notUseTitleOfRelation
                $excep = ($number === FALSE);
                $excep2 =  ($number == FALSE);
                    ...
                    if ($action == 'admin')
                    {
                        ...
                        $this->attributes[$number] =
                                array(
                                    'name' => $relation[2],
                                    'type' => 'raw',
                                    'value' => 'CHtml::link(Yii::app()->controller->chopString($data->' . ($excep2 ? $key . '->' . $relatedAttribute : $relation[2]) . '), array("view", "id"=>$data->' . $relation[2] . ', "entity" =>"' . $this->getEntityByModelName($relation[1]) . '"))',
